...
export default function TaskFileUpload({
  ...
  const handleFiles = (newFiles: File[]) => {
    ...
    if (files.length + newFiles.length > maxFiles) {
      showError("Batas File", `Maksimal ${maxFiles} file per submission`)
      return
    }
  ...
  const isNearLimit = files.length >= maxFiles - 2
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <div className="text-xs text-gray-500 dark:text-gray-500">
            Maksimal {maxFiles} file, {maxFileSize}MB per file, total {maxTotalSize}MB
          </div>
        </div>
      </div>
        ...
        <div className="space-y-2">
          ...
          {isNearLimit && !isAtLimit && (
            <div className="flex items-center text-amber-600 text-xs">
              ...
              Hampir mencapai batas maksimal file
            </div>
          )}
