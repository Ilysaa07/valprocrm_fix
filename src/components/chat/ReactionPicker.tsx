  ...
  return (
    <div className="w-72 bg-white border border-gray-200 rounded-lg shadow-xl overflow-hidden">
      <div className="flex border-b border-gray-100">
        <button onClick={() => setTab('emoji')} className={`flex-1 px-3 py-2 text-sm ${tab==='emoji' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}>Emoji</button>
        <button onClick={() => setTab('sticker')} className={`flex-1 px-3 py-2 text-sm ${tab==='sticker' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}>Sticker</button>
      </div>
      {tab === 'emoji' ? (
        <div>
          <div className="p-2 border-b border-gray-100">
            <input className="w-full px-2 py-1 text-sm border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-blue-500" placeholder="Cari emoji..." value={query} onChange={(e) => setQuery(e.target.value)} />
          </div>
          <div className="p-2 grid grid-cols-8 gap-1 max-h-56 overflow-y-auto">
            {emojis.map((e) => (
              <button key={e} className="text-xl hover:bg-gray-100 rounded" onClick={() => onPickEmoji(e)}>
                {e}
              </button>
            ))}
          </div>
        ...
        <div className="p-3 grid grid-cols-3 gap-2 max-h-64 overflow-y-auto">
          {stickers.length === 0 && (
            <button onClick={async () => setStickers(await fetchStickers())} className="col-span-3 text-sm text-blue-600 hover:text-blue-800 underline">Muat stiker</button>
          )}
          {stickers.map((s) => (
            <button key={s.url} type="button" onClick={() => onPickSticker({ fileUrl: s.url, fileType: s.type, fileName: s.name, fileSize: 0 })} className="w-20 h-20 bg-gray-50 hover:bg-gray-100 rounded-lg flex items-center justify-center" title={s.name}>
